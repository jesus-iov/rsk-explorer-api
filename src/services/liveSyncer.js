...
export async function liveSyncer (syncStatus, confirmationsThreshold) {
  ...
  setInterval(() => {
    if (syncStatus.staticSyncerStarted) { // to allow staticSyncer to save latestBlock - confirmationsThreshold (last immutable block)
...
async function updateDbTipBlock (number, blocksBase, { initConfig, log }, confirmationsThreshold) {
  ...
  try {
    while (!chainsParentBlockFound && gap > 0) {
      ...
      if (dbChainPreviousBlock) {
        console.log({ newChainPreviousBlock: newChainPreviousBlock.hash, dbChainPreviousBlock: dbChainPreviousBlock.hash })
